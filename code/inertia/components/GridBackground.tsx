  ...
  return (
    <div className="absolute inset-0 z-0 grid overflow-hidden">
      <motion.div
        ...
        style={{
          backgroundImage: `
                    linear-gradient(to right, ${backgroundTheme.gradient} 3px, transparent 1px),
                    linear-gradient(to bottom, ${backgroundTheme.gradient} 3px, transparent 1px)
                    `,
      ...
      {props.iconsDensity && props.type && (
        ...
        >
          <Particles iconsDensity={props.iconsDensity} type={props.type} />
        </motion.div>
      )}
      <div
        className={`absolute inset-0 bg-gradient-to-b ${backgroundTheme.overlay} to-transparent`}
      />
    </div>
  )
})
